...
cm_r = LinearSegmentedColormap.from_list(cmap_name_r,np.flip(colors,axis=0),N=1000)
...
def corpus_vectors(corpus,dict_outpath=None,vectorizer='Count',vocab=None,max_features=20000,min_df=2,max_df=0.75):
    ...
    if dict_outpath is not None:
        df_dict = pd.DataFrame(vectorizer.get_feature_names_out(),columns=['words'])
...
def vectors_som(vectors,words,mapsize=[40,60],normalization='var',initialization='pca',verbose=None,som_outpath=False):
    ...
    if os.path.exists(som_outpath)==True:
        ...
        if response == 'y':
            ...
            som.train(n_job = 1,verbose=verbose)#,train_rough_len=100,train_finetune_len=200)
...
def persona_talks_grid(cb_vectors,corpus,som_outpath,gridsize=[50,50],som_verbose='info',plot_grid=False,save_plot=False,save_plot_path=False,masksize=[50,50],figsize=(40,40)):
    if os.path.exists(som_outpath)==True:
        ...
        if response == 'y':
            ...
            print ("Topographic error = %s\n Quantization error = %s" % (som.calculate_topographic_error(), som.calculate_quantization_error()))
    ...
    if plot_grid==True:
        ...
        fig = plt.figure(figsize=figsize)
        grid = ImageGrid(fig, 111,  # similar to subplot(111)
                         nrows_ncols=(gridsize[0], gridsize[1]),  # creates 2x2 grid of axes
                         axes_pad=0.00,  # pad between axes in inch.
                         )
        for i, ax in zip(range(gridsize[0]*gridsize[1]),grid):
            try:
                df_cb_grid_sel = df_cb_grid[df_cb_grid['bmu']==i].iloc[0]
                ax.imshow(df_cb_grid_sel.iloc[3:].values.reshape(masksize[0],masksize[1]).astype('float'),cmap=cm,alpha=1)
                ax.axis('off')
            except:
                ax.axis('off')
            pass
        ...
        if save_plot==True:
            print('... saving figure.')
            fig.savefig(save_plot_path,dpi=300,bbox_inches = 'tight',pad_inches = 0)
        else:
            pass
    else:
        pass
...
def query_talks_grid(talks_grid,query_bmu,max_masks=10,figsize=(20,40),masksize=[75,75]):
    ...
    fig = plt.figure(figsize=(figsize[0], figsize[1]))
    grid = ImageGrid(fig, 111,  # similar to subplot(111)
                     nrows_ncols=(int(talks_grid_sel.shape[0]/5)+1, 5),  # creates 2x2 grid of axes
                     axes_pad=0.05,  # pad between axes in inch.
                     )   
    for ax, im, t,a in zip(grid,talks_grid_sel.iloc[:,3:].values,talks_grid_sel.title,talks_grid_sel.author):
        ax.xaxis.set_label_position('top')
        ax.text(30,5,t+'\n ('+a+')',fontsize=10,**csfont,color='white',horizontalalignment='center',verticalalignment='center')
